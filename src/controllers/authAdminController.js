...
const generateAccessToken = (user) => jwt.sign(user, process.env.ACCESS_SECRET_ADMIN, { expiresIn: "15m" });
const generateRefreshToken = (user) => jwt.sign(user, process.env.REFRESH_SECRET_ADMIN, { expiresIn: "7d" });
...
manageBackups:true
  ...
  const login = async (req, res) => {
    ...
    try {
      ...
      const passwordMatch = isVerified ? password === employee.password : await bcrypt.compare(password, employee.password);
      if (!passwordMatch) {
        const message = await translate('Invalid email or phone or password or role', { to: lang });
        return res.status(401).json({
          status: false,
